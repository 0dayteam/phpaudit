...
// WithoutIgnoreDir set up  ignore dir filter
func WithoutIgnoreDir(ignoreDir []string) Set {
	return func(f *finder) {
		f.ignoreDir = ignoreDir
	}
}
	...
	go func(f *finder) {
		_ = filepath.Walk(f.root, func(n string, fi os.FileInfo, err error) error {
			...
			if err == nil {
				...
				// If it in ignore dir,  will ignore
				if fi.IsDir() {
					for _, dir := range f.ignoreDir {
						if filepath.Base(n) == dir {
